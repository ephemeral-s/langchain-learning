...
tools = [search]
model = init_chat_model(model="gpt-5.6-terra", base_url="https://api.jiekou.ai/openai", temperature=0)
...
def tool_node(state: MessagesState):
    ...
    for tool_call in state["messages"][-1].tool_calls:
        ...
        obs = tool.invoke(tool_call["args"]) # 调用工具
        result.append(ToolMessage(content=obs, tool_call_id=tool_call["id"])) # 将执行结果包装乘ToolMessage
